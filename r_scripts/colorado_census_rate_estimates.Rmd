...
Contains descriptive epidemiology of cardiopulmonary emergency department (ED) and urgent care visits in Colorado from 2010 to September of 2015. We've decided to limit the period of estimates from 2010 to 2014. Using the 2014 census estimates Sheena.
...
census_rate_period <- co_hosp_census %>% 
  ...
  cardiopulm_per_100_5y = (cardiopulm_n/total_pop5y)*100,
  cvd_per_100_5y = (cvd_n/total_pop5y)*100,
  resp_per_100_5y = (resp_n/total_pop5y)*100)

# head of these calculations
head(census_rate_period)
```

Checking distribution of the rates. Add small multiples or colors.

```{r census rate distribution}
p <- ggplot(data = census_rate_period, aes(x= cvd_per_100_5y)) +
  geom_density()
