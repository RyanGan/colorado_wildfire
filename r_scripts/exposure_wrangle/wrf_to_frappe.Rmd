...
```{r new_bbox}
# new bbox
frappe_bbox <- c(-109.2346, 38.3687, -101.3326, 41.22053)
# new grid
...
```{r extract_coords}
# new coordinates
new_coords <- st_coordinates(st_centroid(new_grid))
# wrfgrid id vector
new_wrfid <- new_grid$WRFGRID_ID
# frapp grid
frappe_grid <- data.frame(new_wrfid, new_coords) %>% 
  rename(WRFGRID_ID = new_wrfid, long = X, lat = Y)

# check new grid
ggplot(frappe_grid, aes(x=long,y=lat)) +
