...
- I could create an R package to be shared with Rish to make this whole process easier. I could write out the functions created here to be part of a package so it's easier to read.

- I am going to talk with Ander about interpreting these data to make sure I'm on the right path. If you have discussions with Antonio, see if he'd agree with this approach.

- I'm going to run these same analyses, assigning PM at the Zip level and grid level. I will also consider restricting analyzes to front range areas where Jeff thinks the kriged surfaces are more accurate (which would exclude a lot of rural counties).