...
co_resp_cvd_death_list <- co_mortality_list[c(1,4)] 
...
# co resp cvd hosp list
co_resp_cvd_hosp_list <- co_morbidity_cc_list[c(1,6)]
...
death_outcome <- names(co_resp_cvd_death_list)

# time series of weekly death and rate for cardiovascular and respiratory
death_ts <- map2(.x = co_resp_cvd_death_list,  .y = death_outcome, 
...
```{r hosp_rate}
# hosp_names
hosp_outcome <- names(co_resp_cvd_hosp_list)
# time series of weekly death and rate for cardiovascular and respiratory
hosp_ts <- map2(.x = co_resp_cvd_hosp_list,  .y = hosp_outcome, 
                 ~ filter(.x, outcome == 1 & fips %in% front_range_fips) %>% 
...
This plot needs work too. I also want to add a outcome numbers for the months included in the analyses tables. Also note that hospitalizations end at the end of September 2015 as this is when the nationwide switch from ICD9 to ICD10 occured.
...
```{r death_pm_join}
# assign death outcome
death_outcome <- names(co_mortality_list)
# assign outcome names to each dataframe in list
co_death_list_pm <- co_mortality_list %>% 
  ...
  map(~ mutate(., outcome = as.numeric(as.character(outcome)),
      ...
      mutate(grid_pm_miss = ifelse(is.na(pm25_grid), 1, 0),
        cty_pm_miss = ifelse(is.na(pm25_c_10u),1, 0))) %>% 
  # add outcome name to each dataframe
  map2(.x = ., .y = death_outcome, ~mutate(.x, out_name = .y)) 
```

Summary of counts and missing PM2.5 values.

```{r death_sum_of_missing}
# estimate deaths 
death_counts <- co_death_list_pm %>% 
  map_dfr(. , function(df){
    counts <- df %>% 
      filter(outcome == 1) %>% 
      group_by(out_name) %>% 
      summarise(n_events = n(), n_pm_g_miss = sum(grid_pm_miss), 
                n_pm_c_miss = sum(cty_pm_miss)) %>% 
      mutate(perc_grid_miss = (n_pm_g_miss/n_events)*100)
    })
# print death counts
death_counts
```
...
# remove co_hosp_list to save space
rm(co_hosp_list)
```

Counts of inpatient hosptial events and missing PM2.5
```{r hosp_summary}
# estimate deaths 
hosp_counts <- co_hosp_list_pm %>% 
  map_dfr(. , function(df){
    counts <- df %>% 
      filter(outcome == 1) %>% 
      group_by(out_name) %>% 
...
Same day associations not considering season.

```{r death_same_day}
death_outcome2 <- rep(death_outcome, each = 4)
# distributed lag function
death_same_day_grid <- co_death_list_pm %>% 
  map_dfr(. , function(df){
    # attempt a split to do seasonal analyses
    season_list <- df %>% split(.$season)
    season <- names(season_list)
    # run second map
    season_results <- map_dfr(season_list, function(df2){
    mod <- clogit(outcome ~ pm25_g_10u + temp_f_grid + strata(id), data = df2)
    ...
    # add outcome name to each dataframe
  cbind(death_outcome2, .) %>% 
  mutate(season = factor(season, 
                         levels = c('spring', 'summer','fall', 'winter')))

# print results
print(death_same_day_grid)
```

Same day death plot.

```{r death_results}
ggplot(data=death_same_day_grid, aes(x=season, y=estimate, color=season)) +
  geom_point() + 
  geom_errorbar(aes(ymin=lower95, ymax=upper95),width = 0.2) +
  geom_hline(yintercept = 1, linetype=2) +
  facet_wrap(~death_outcome2, scales = 'free_y') +
  ylab(expression(paste("Odds Ratio for 10µg/m"^3, " Increase in PM"[2.5]))) +
  xlab("Season") +
  ggtitle("Mortality by Season") +
   # plot theme
...
## Inpatient Hospitalization and PM~2.5~ Association

Same day associations not considering season.

```{r hosp_same_day}
hosp_outcome2 <- rep(hosp_outcome, each = 4)
# distributed lag function
hosp_same_day_grid <- co_hosp_list_pm %>% 
  map_dfr(. , function(df){
    # attempt a split to do seasonal analyses
    season_list <- df %>% split(.$season)
    season <- names(season_list)
...
# print results
print(hosp_same_day_grid)
```

Same day hospital plot.

```{r hosp_results, fig.height=6, fig.width=6}
hosp_plot <- ggplot(data=hosp_same_day_grid, aes(x=season, y=estimate, color=season)) +
  geom_point() + 
  geom_errorbar(aes(ymin=lower95, ymax=upper95),width = 0.2) +
  geom_hline(yintercept = 1, linetype=2) +
  facet_wrap(~hosp_outcome2, scales = 'free_y', ncol = 3) +
  ylab(expression(paste("Odds Ratio for 10µg/m"^3, " Increase in PM"[2.5]))) +
  xlab("Season") +
  ggtitle("Inpatient Hospitalization") +
   # plot theme
    theme(panel.background = element_rect(fill = 'white', colour = 'black'),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
