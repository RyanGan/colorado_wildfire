...
co_death_list <- casecross_list[c(1,3,4,5,7,9)]
...
names(icd9_outcomes)
# co hosp list
co_hosp_list <- co_morbidity_cc_list[c(1,2,3,6,7,8,10,11)]
# removing 
...
Model distributed lag results based on best fit by AIC. Limiting April through October when it's warm out. I guess I could do a seasonal strata. I should actually define seasons better based on date rather than month. Justification would be the Peng seasonal paper which finds the strongest effect in summer months, and more importantly, no effect during winter months. I've seen my effects wash out when I include cold months. Peng et al argues for different seasonal sources, but I'd also argue for behavioral differences such as leaving windows open, or going outside more, etc. 
...
Joining PM~2.5~ estimates to hosptializations list. I will limit analyses to April through October in the model step.
...
I don't see an association with PM~2.5~ and either COPD or asthma. It also has an inverse association with respiratory. Colorado seems to have some interesting issues I think that I may not have quite yet flushed out. I vaguely recall Kirk and Stephanie saying there were some potential biases in the earlier years of CHA (2010). Also, it's possible that air quality warnings are helpful. Hard to know if a unique observation adheres to this.
...
mort_dl_fit <- map(co_death_list_pm, function(x){
  ...
  smk_mat <- as.matrix(select(data, contains("smk5_hms_g")))
  ...
  outcome_fit <- apply(df_combo, 1, function(fit){
      ...
      # define lagged basis spline for pm
      pm_smk_exp_b <- ns(0:(ncol(pm_mat)-1), df = as.numeric(fit[[1]]), 
                         intercept = T)
      # pm basis
      pm_basis <- pm_mat %*% p_exp_b
    ...
    # fit
    # define lagged basis spline for pm
    p_smk_exp_b <- ns(0:(ncol(pm_mat)-1), df = as.numeric(fit[[1]]), intercept = T)
    # pm basis
    pm_basis <- pm_mat %*% p_exp_b
