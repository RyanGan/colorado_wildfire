...
## Note 2020-04-18:
Ideally I'd like to give the exact coordinates for the bounding box for Kate, 
but I don't remember how to exacly subset. I've printed out the Lon/Lat coords
of the grid cells I used in air pollution measures for the front range. Keep
in mind I projected it as a WGS 84 to align with other shapefiles. But it looks
like the bounding box cells would be 
Top left: -105.39, 40.63
Top right: -104.62, 40.67
Bottom left: -105.24, 38.59
Bottom right: -104.49, 38.62
...
vpinset <- grid::viewport(width = 0.25, height = 0.25, x = 0.725, y = 0.85)
print(study_map, vp = vpmain)
print(inset_map, vp = vpinset)
```

Saving high quality map images for paper.
```{r save_map_w_inset}
# save map as png high res
png(
  filename = "../analysis/figures/fig1_study_map.png", 
  width = 8, 
  height = 6, 
  units = 'in',
  res = 300
...
grid::grid.newpage()
vpmain <- grid::viewport(width = 1, height = 1, x = 0.5, y = 0.5)
vpinset <- grid::viewport(width = 0.25, height = 0.25, x = 0.72, y = 0.85)
print(study_map, vp = vpmain)
print(inset_map, vp = vpinset)
dev.off()
...
# save map as png high res
pdf(
  file = "../analysis/figures/fig1_study_map.pdf", 
  width=8, 
  height=6
  )
grid::grid.newpage()
...
ggsave(
  filename = '../analysis/figures/fig3_pm.pdf',
  plot = pm_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/fig3_pm.png',
  plot = pm_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
### Cumulative effect for a 10 ug/m^3 of smoke exposure over 3 days

#### 2020-12-05: REVIEWER COMMENT
A reviewer noted that the 3 degrees of freedom lag looks smooth. I've modified
the code below to look at each outcome using 2,3, and 4 degrees of freedom for 
the splines used to construct the distributed lag basis.
...
#### Note 2020-12-05: Addressing reviewer comment
I added an internal lapply functuion to run the lagged models for 2, 3, and 4
degrees of freedom. I've saved the AIC and BIC for each model as well.
...
# repeate the 4 outcomes 2 times for lagged and cumulative, and repeate 3 for each df
rep_times <- 4*2*3
# repeat outcome multiple times
...
dl_results  <- lapply(co_hosp_list, function(x) {
  ...
  # second lapply for model using different degrees of freedom for spline
  results_list <- lapply(2:4, function(deg_free) {
    # define lagged basis spline
    exp_b <- splines::ns(0:(ncol(pm_mat)-1), df = deg_free, intercept = T)
  ...
  }) # end degrees of freedom iteration lapply
  # bind list to dataframe
  lag_est <- do.call(rbind, results_list)
  return(lag_est)
  }) %>%  #end outcomes lapply
  # bind rows
  map_dfr(.,rbind) %>% 
...
#### Note 2020-12-05: Results for all versions of lag models

I've add an interactive plot so it's easier to sort through. I think the take
away across all estimates that if you want to. In short, I don't think either
2, 3, or 4 degrees of freedom splines produce that different of results. I think
3 degrees of freedom was chosen as it was flexible enough without too many knots.

However, we could pick results by best (lowest) AIC or BIC for each outcome if
that would address reviewer comments. In that case, it would be splines with 2 df.
After this table, I'll keep results for the original 3 degrees of freedom model 
unless we decide otherwise. 

```{r sensitivity_dl_hosp_results_table}
dl_results %>% 
  mutate_at(vars(odds_ratio:bic), ~round(., 3)) %>% 
  DT::datatable(., filter = 'top') # DT interactive table in javascript
```

Cumulative results for the 3 degree of freedom spline.

```{r cumulative_cardiopulmonary_results}
# get the cumulative effect over 6 days
cumulative_results <- dl_results %>% 
  filter(type == 'cumulative' & time == 3 & spline_df == 3) %>% 
...
ggsave(
  filename = '../analysis/figures/figS2_hosp_cumulative.pdf',
  plot = plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/figS2_hosp_cumulative.png',
  plot = plot,
  height = 6,
  width = 8,
  unit = 'in',
...
Creating a plot showing the individual lagged day effects on cardiopulmonary 
hospitalizations. This is a constrained distributed lag model, meaning all 
lagged days are accounted for in the model.

### Note 2020-12-03: 3 degrees of freedom model
Sticking with 3 df spline models for now unless we decide otherwise.
...
ggsave(
  filename = '../analysis/figures/figS1_dl_hosp.pdf',
  plot = dl_hosp_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/figS1_dl_hospe.png',
  plot = dl_hosp_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
### NOTE 2020-12-05: Reviewer feedback that this is redundant 
Ander suggested removing based on reviewer comments. I would agree that it 
doesn't add much except maybe internal checks.
...
# save unconstrained lag plot for morbiity
ggsave(
  filename = '../analysis/figures/fig4_unconst_lag.pdf',
  plot = uc_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/fig4_unconst_lag.png',
  plot = uc_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
# save plot sup fig 3
ggsave(
  filename = '../analysis/figures/figS3_hosp_sexstrata.pdf',
  plot = sex_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/figS3_hosp_sexstrata.png',
  plot = sex_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
ggsave(
  filename = '../analysis/figures/figS4_hosp_agestrata.pdf',
  plot = age_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/figS4_hosp_agestrata.png',
  plot = age_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
rep_times <- 4*2*3 # 3 days and 2,3,4 df
...
#### Note 2020-12-05: Results for all versions of mortality lag models

Same thing as the distributed lag hospital results above. 
I've add an interactive table so it's easier to sort through. I think the take
away across all estimates that if you want to. In short, I don't think either
2, 3, or 4 degrees of freedom splines produce that different of results. I think
3 degrees of freedom was chosen as it was flexible enough without too many knots.
...
```{r death_sensitivity_dl_hosp_results_table}
dl_death_results %>% 
  mutate_at(vars(odds_ratio:bic), ~round(., 3)) %>% 
  DT::datatable(., filter = 'top') # DT interactive table in javascript
```
...
```{r death_cumulative}
cumulative_death_results <- dl_death_results %>% 
  filter(type == 'cumulative' & time == 3 & spline_df == 3) %>% 
  dplyr::select(-strata, -time) %>% 
...
ggsave(
  filename = '../analysis/figures/figS6_mortality.pdf',
  plot = cumulative_death_plot,
  height = 6,
  width = 8,
  unit = 'in'
)
...
ggsave(
  filename = '../analysis/figures/figS6_mortality.png',
  plot = cumulative_death_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
But there is too much noise at this point I think to make any conclusions if there is an association or not
with smoke and CVD or respiratory deaths in this data.
...
ggsave(
  filename = '../analysis/figures/fig5_mortality.pdf',
  plot = death_uc_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/fig5_mortality.png',
  plot = death_uc_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
ggsave(
  filename = '../analysis/figures/figS7_mortality_sexstrata.pdf',
  plot = sex_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/figS7_mortality_sexstrata.png',
  plot = sex_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
ggsave(
  filename = '../analysis/figures/figS8_mortality_agestrata.pdf',
  plot = age_plot,
  height = 6,
  width = 8,
  unit = 'in'
...
ggsave(
  filename = '../analysis/figures/figS8_mortality_agestrata.png',
  plot = age_plot,
  height = 6,
  width = 8,
  unit = 'in',
...
waldo_hosp_cont <-lapply(co_hosp_list, function(x){
  ...
  data <- x %>% 
    ...
    # NOTE 2020-06-20 changed to match other date restrictions
...
#### Early Season Fires
Restricting to Early Season Fires June 1 to July 31st.

```{r transport_hosp_results_early}
rep_times <- 2*4
out_rep <- rep(outcome, each = rep_times)

trans_hosp_cont_early <-lapply(co_hosp_list, function(x){
  # output dataframe from list
  data <- x %>% 
    filter(date >= '2015-05-01' & date <= '2015-07-31') %>% 
    mutate(date = as.Date(date),
           outcome = as.numeric(as.character(outcome))) %>%
    # remove missing lagged data
    filter(!is.na(gpm_smk10unit_lag5))
  ...
  # bind in outcome names
  bind_cols(data.frame(out_rep), .) %>% 
  mutate(event = 'Transport 2015: May 1st to July 31st')
```

```{r trans_death_cont_early}
rep_times <- 2*4

cod <- rep(names(co_death_list[c(1,4)]), each=rep_times)

trans_death_cont_early <- lapply(co_death_list[c(1,4)], function(x){
  # output dataframe from list
  data <- x %>%     
    filter(date >= '2015-05-01' & date <= '2015-07-31') %>% 
    mutate(date = as.Date(date),
...
Local fires early
```{r local_hosp_results_early}
rep_times <- 2*4 # 3 day
out_rep <- rep(outcome, each = rep_times)

local_hosp_cont_early <-lapply(co_hosp_list, function(x){
  # output dataframe from list
  data <- x %>% 
    filter(date >= '2012-05-01' & date <= '2012-07-31') %>% 
    mutate(date = as.Date(date),
           outcome = as.numeric(as.character(outcome))) %>%
    # remove missing lagged data
    filter(!is.na(gpm_smk10unit_lag5))
  ...
  # bind in outcome names
  bind_cols(data.frame(out_rep), .) %>% 
  mutate(event = 'Local 2012: May 1st to July 31st')
```

```{r local_deaths_early}
rep_times <- 2*4
cod <- rep(names(co_death_list[c(1,4)]), each=rep_times)

local_death_cont_early <- lapply(co_death_list[c(1,4)], function(x){
  # output dataframe from list
  data <- x %>%     
    filter(date >= '2012-05-01' & date <= '2012-07-31') %>% 
    mutate(date = as.Date(date),
...
#### Late Season Fires
Restricting to Late Season Fires August 1st to Oct 31st.

```{r transport_hosp_results_late}
rep_times <- 2*4
out_rep <- rep(outcome, each = rep_times)

trans_hosp_cont_late <-lapply(co_hosp_list, function(x){
  # output dataframe from list
  data <- x %>% 
...
```{r trans_death_cont_late}
rep_times <- 2*4

cod <- rep(names(co_death_list[c(1,4)]), each=rep_times)

trans_death_cont_late <- lapply(co_death_list[c(1,4)], function(x){
  # output dataframe from list
  data <- x %>%     
    filter(date >= '2015-08-01' & date <= '2015-10-31') %>% 
    mutate(date = as.Date(date),
...
Local fires late
```{r local_hosp_results_late}
rep_times <- 2*4 # 3 day
out_rep <- rep(outcome, each = rep_times)

local_hosp_cont_late <-lapply(co_hosp_list, function(x){
  # output dataframe from list
  data <- x %>% 
    filter(date >= '2012-08-01' & date <= '2012-10-31') %>% 
    mutate(date = as.Date(date),
...
local_death_cont_late <- lapply(co_death_list[c(1,4)], function(x){
  # output dataframe from list
  data <- x %>%     
    filter(date >= '2012-08-01' & date <= '2012-10-31') %>% 
    mutate(date = as.Date(date),
...
```{r hosp_event_plot_late}
# bind event dataframes together
hosp_event_late <- bind_rows(waldo_hosp_cont, hipark_hosp_cont, local_hosp_cont_late,
                        trans_hosp_cont_late) %>% 
  filter(type == 'cumulative' & time == 3) %>% 
  dplyr::select(-strata, -time) %>% 
  mutate(outcome = forcats::fct_relevel(out_rep, out_order),
         group = as.factor(if_else(outcome %in% out_order[1:5],
         ...
         event = forcats::fct_relevel(event, c('Waldo Canyon 2012', 'High Park 2012',
                                               ...
                                               'Transport 2015: August 1st to October 31st'))) %>% 
  # filter out acute bronchitis
  filter(outcome != 'Acute Bronchitis')

# plot
hosp_event_plot_late <- ggplot(data = hosp_event_late, 
                          aes(x=outcome, y=odds_ratio, color = group)) +
  geom_point() +
  geom_errorbar(aes(ymin=lower_95, ymax=upper_95), width = 0.3) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "red") +
...
```{r mortality_event_plot_late}
# bind event dataframes together
death_event_late <- bind_rows(waldo_death_cont, hipark_death_cont, local_death_cont_late,
                        trans_death_cont_late) %>% 
  filter(type == 'cumulative' & time == 3) %>% 
  dplyr::select(-strata, -time) %>% 
  mutate(outcome = as.factor(if_else(cod == 'resp', 
                                     'Respiratory', 'Cardiovascular')),
...
# plot
death_event_plot_late <- ggplot(data = death_event_late, 
                          aes(x=outcome, y=odds_ratio, color = outcome)) +
  geom_point() +
  geom_errorbar(aes(ymin=lower_95, ymax=upper_95), width = 0.3) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "red") +
...
print(death_event_plot_late)
```

### Early Season vs. Late Season Fires

Hospitalization plots.

```{r hosp_event_plot_v2}
# bind event dataframes together
hosp_event_v2 <- bind_rows(local_hosp_cont_early, trans_hosp_cont_early, 
                           local_hosp_cont_late, trans_hosp_cont_late) %>% 
  filter(type == 'cumulative' & time == 3) %>% 
  dplyr::select(-strata, -time) %>% 
  mutate(outcome = forcats::fct_relevel(out_rep, out_order),
         group = as.factor(if_else(outcome %in% out_order[1:5],
         ...
         event = factor(
           event, 
           levels = c(
             'Local 2012: May 1st to July 31st',
             'Transport 2015: May 1st to July 31st',
             'Local 2012: August 1st to October 31st',
             'Transport 2015: August 1st to October 31st'
             )
           )
...
ggsave(
  filename = '../analysis/figures/fig6_local_vs_transport.pdf',
  plot = hosp_event_plot_v2,
  height = 6,
  width = 8,
  unit = 'in'
)
...
ggsave(
  filename = '../analysis/figures/fig6_local_vs_transport.png',
  plot = hosp_event_plot_v2,
  height = 6,
  width = 8,
  unit = 'in',
...
Printing out effect estimates in plots.

Table of age-specific associations.

```{r transport_age_specific_death_tabs}
knitr::kable(dplyr::select(cumulative_age_death_strata, 
                    outcome, age_long, odds_ratio:upper_95) %>%
    mutate_at(vars(odds_ratio:upper_95), ~round(.x, 3)),
             caption = 'Age-specific associations between smoke and underlying cause of death')
...
Print out table of effect estimates.
```{r hosp_event_effect_estimates_tabs}

knitr::kable(dplyr::select(hosp_event_v2, 
                    event, outcome, type, odds_ratio:upper_95) %>%
    mutate_at(vars(odds_ratio:upper_95), ~round(.x, 3)),
             caption = 'Local and Transport Hospitalization Effect Estimates')
```

Mortality plot.

```{r mortality_event_plot_late_v2}
# bind event dataframes together
death_event_v2 <- bind_rows(local_death_cont_early, trans_death_cont_early, 
                            local_death_cont_late, trans_death_cont_late) %>% 
  filter(type == 'cumulative' & time == 3) %>% 
  dplyr::select(-strata, -time) %>% 
  mutate(outcome = as.factor(if_else(cod == 'resp', 
                                     'Respiratory', 'Cardiovascular')),
...
```
Print out table of mortality effect estimates.
```{r mortality_event_effect_estimates_tabs}
knitr::kable(dplyr::select(death_event_v2, 
                    event, outcome, type, odds_ratio:upper_95) %>%
    mutate_at(vars(odds_ratio:upper_95), ~round(.x, 3)),
             caption = 'Local and Transport Mortality Effect Estimates')
```
