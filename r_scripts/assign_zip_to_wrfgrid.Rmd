...
pi_matrix <- function(grid_sf, grid_id, prop_int_df, poly_id){
  ...
  output_df <- grid_sf %>% 
    select(!!grid_id) %>%
    semi_join(prop_int_df, by = as.character(grid_id[2])) %>% 
    mutate(id = as.factor(paste0("id", !!poly_id))) %>%
    # spread
    spread(id, proportion) %>% 
    # mutate missing to 0 at each fips var
    mutate_at(vars(contains("id")), funs(ifelse(is.na(.), 0,
              ifelse(.>1, 1, .)))) %>% 
    # remove idNA
    select(-idNA, -!!grid_id, -!!poly_id)
...
co_census_df <- proportion_intersect(poly = colorado_census, poly_id = TRACTCE,
                                     grid = wrf_grid, grid_id = WRFGRID_ID)
...
census_pi_matrix <- pi_matrix(grid_sf = wrf_grid, grid_id = WRFGRID_ID, 
                              prop_int_df = co_census_df, poly_id = TRACTCE)

12952, 12953
check <- co_census_df %>% slice(13160:13161)
  
summary(co_census_df)
```
